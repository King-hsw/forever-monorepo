...
/**
 * 带行号、语法高亮及顶栏工具条（语言选择 + 复制）的代码块。
 *
 * 在 CodeBlockLowlight 基础上通过自定义 NodeView 渲染：行号栏（gutter）和
 * 顶栏（toolbar）都挂在 <code> 内容区**外部**，与可编辑内容完全隔离。之前尝试过把行号作为
...
export const CodeBlockLineNumbers = CodeBlockLowlight.extend({
  addNodeView() {
    ...
    const lowlight = this.options.lowlight as { listLanguages?: () => string[] } | undefined;
    ...
    return ({ node, editor, getPos }) => {
      ...
      // 顶栏：三圆点 + 语言选择 + 复制按钮，结构与配色对齐渲染端的
      // .md-code-block__bar（见 styles/prose.less），保证所见即所得；
      ...
      return {
        ...
        // 顶栏上的交互（点开下拉、点击复制）不交给 ProseMirror 处理，
