...
function protectPipes(node: JSONContent): JSONContent {
  if (node.type === 'text') {
    ...
    return { ...node, text: node.text.replaceAll('|', PIPE_SENTINEL) };
  }
 ...
 * 清洗阶段被平铺（见 utils/pasteTable.ts），确保编辑态所见即保存所得。
 */
export const TableMarkdownSafe = Table.extend({
  renderMarkdown(node: JSONContent, helpers: MarkdownRendererHelpers): string {
    ...
    return markdown.includes(PIPE_SENTINEL) ? markdown.replaceAll(PIPE_SENTINEL, '\\|') : markdown;
  },
});
